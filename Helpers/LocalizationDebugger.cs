		...
		public static void DrawDebugger<T>(LocalizationHandler<T> handler) where T : LocalizationScriptableBase
		{
			using (new GUILayout.VerticalScope(GUI.skin.box))
			{
				...
				if (foldout)
				{
					...
					DrawPrefStringField("Type Pref Language", $"{LocalizationConstants.LANGUAGE_KEY_PREFIX}{type.Name}");
					if (handler != null)
					{
						EditorGUILayout.Space();
						handler.hasSelectedALanguage = EditorGUILayout.Toggle("Has Type Language", handler.hasSelectedALanguage);
						...
						using (new GUILayout.HorizontalScope())
						{
							...
							if (GUILayout.Button(GUIContent.none, EditorStyles.popup, GUILayout.Width(20)))
							{
								...
								foreach(var key in handler.selectedLanguage.localizedContent.Select(lc => lc.keyName).Distinct().OrderBy(s => s))
